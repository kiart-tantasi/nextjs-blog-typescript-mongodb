...
export default function CardUI(props:{articleCard:ArticleCard, bin?: boolean}) {
    ...
    <article className={styles.card}>
        <Card sx={{borderRadius:{xs: 0, sm: 0.5, md: 1}}}>
            ...
            {/* TITLE */}
            <Link href={href}><a className={styles["title-a-tag"]}><h1 className={styles["title-h1-tag"]}>{props.articleCard.title}</h1></a></Link>
            ...
            <div className={styles["date-readmore"]}>
                <div className={styles["date-div"]}>
                    <Typography sx={{fontSize:11}} color="text.secondary">{new Date(props.articleCard.date).toLocaleDateString("th-TH",{day:"numeric", month:"long", year:"numeric"})}</Typography>
                </div>
