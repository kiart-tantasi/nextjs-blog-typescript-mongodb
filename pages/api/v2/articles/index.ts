...
import { collectionName } from '../../../../interfaces/constant'
...
export default isAuthenticated(async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    // POST A WORKSPACE ARTICLE TO PUBLIC
        ...
        try {
            ...
            const artclesCollection = db.collection(collectionName.articles)
            const isDuplicate = await artclesCollection.findOne({ slug: slug })
            if (isDuplicate) {
            ...
            const insertResult = await artclesCollection.insertOne(articleToPostToPublic)
    ...
    // DELETE AN ARTICLE (MOVE TO BIN)
        ...
        try {
            ...
            if (
                !articleNoTransformed ||
                !articleNoTransformed.title ||
                !articleNoTransformed.desc ||
                !articleNoTransformed.markdown ||
                !articleNoTransformed.img ||
                !articleNoTransformed.alt ||
                !articleNoTransformed.date ||
                !articleNoTransformed.category ||
                !articleNoTransformed.slug
            )
                throw new Error('article not found in the category or some information is missing.')
